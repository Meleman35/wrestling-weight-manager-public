-- Direct, email-bound organization invitations. No team, affiliate or billing writes.
create table private.organization_leadership_invitations (
 id uuid primary key,
 organization_id uuid not null references public.organizations(id),
 invited_email text not null,
 kind text not null check (kind in ('administrator','position')),
 position_id uuid references private.organization_positions(id),
 position_revision integer,
 access_role text,
 ...
 check ((kind='administrator' and position_id is null and access_role is null)
     or (kind='position' and position_id is not null and position_revision is not null))
);
create index organization_leadership_invitations_org_idx
 on private.organization_leadership_invitations(organization_id,created_at desc);
alter table private.organization_leadership_invitations enable row level security;
revoke all on private.organization_leadership_invitations from public,anon,authenticated;

create function private.organization_leadership_invites(q jsonb) returns jsonb
...
begin
 ...
 if act not in ('context','create','preview','accept','revoke','email_context','remove_admin') or act is null then raise exception 'Unknown invitation action.'; end if;
 ...
 if act in ('preview','accept') then
  ...
  if inv.kind='administrator' then
 ...
 if act='create' then
  ...
  if rid is null or kind_text not in ('administrator','position') or kind_text is null then raise exception 'Choose an invitation role.'; end if;
  ...
  if kind_text='administrator' then
  ...
  if exists(select 1 from private.organization_leadership_invitations where organization_id=o and status='pending' and expires_at>now()
   and ((kind_text='position' and position_id=pos_id) or (kind_text='administrator' and kind='administrator' and invited_email=mail))) then
...
$$;
revoke all on function private.organization_leadership_invites(jsonb) from public,anon,authenticated;
grant execute on function private.organization_leadership_invites(jsonb) to authenticated;
create function public.organization_leadership_invites(p_request jsonb) returns jsonb
language sql security invoker set search_path='' as $$ select private.organization_leadership_invites(p_request); $$;
revoke all on function public.organization_leadership_invites(jsonb) from public,anon;
grant execute on function public.organization_leadership_invites(jsonb) to authenticated;
