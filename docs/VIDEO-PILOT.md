# Current release: guarded device-only Test scorebook

Version 0.20.46 enables only a time-limited, explicitly granted personal-account test. `test_only` defaults true, real athlete IDs are omitted from capture grants, real-bout/guardian-management APIs are blocked, and cloud/live/billing remain off. Team choice alone does not grant every member recording access. The broader sharing implementation below remains gated.
...
The current draft adds role-aware Match Book entry points in Locker Room and the existing menu for athletes and student managers. Non-coach entry points use scoped recording requests; they do not load the coach match/challenge catalogue. Approved recorders have a Test scorebook with synthetic names and device-only score saves. Camera permission confirmation remains explicit for that test. The current draft also adds athlete-profile **Score & record match**, coach-confirmed event rules, exact event/athlete assignments, linked-parent recording/sharing permission, server-imported bout/opponent identity, private Storage upload records/policies, browser and native foreground TUS upload queues, verified-upload device removal, and athlete/family watch/download. Go live remains disabled because provider ingestion, live playback authorization, and parent live-alert integration are unfinished. A device-recording dot is explicitly labeled not live.

Validation: eleven PGlite authorization/storage groups, seven upload protocol/resume groups, eleven native-adapter browser groups (including one-click assigned-teammate capture), shared scoring/source preservation and JavaScript syntax. These are synthetic fixtures. Native upload Swift passes grammar parsing only; it has not been compiled in Xcode or exercised against real Supabase Storage.
