...
(function(root){
 ...
 function step(prior,rows,now){
  ...
  for(const row of rows){
   ...
   const emit=(key,body)=>{if(!s.seen[key]){s.seen[key]=true;alerts.push({bout_id:id,kind:key,body});}};
   const where=`Bout ${row.bout_number} · ${row.mat?'Mat '+row.mat:'Mat pending'}`;
   const fresh=now-stamp<=600000&&stamp<=now+60000&&!row.stale;
   if(fresh){
    if(old&&s.mat!==row.mat)emit('mat:'+version+':'+stamp,where+' · Mat assignment changed');
    ...
    if(row.visibility!=='mat_only'&&active.has(row.status)){
     if(labels[row.status])emit('status:'+row.status,where+' · '+labels[row.status]);
     const eta=Date.parse(row.estimated_start),minutes=(eta-now)/60000;
     if(old&&Number.isFinite(eta)&&s.eta&&Math.abs(eta-s.eta)>=120000)emit('reschedule:'+version+':'+stamp,where+' · Estimated start changed');
     if(row.status!=='on_mat'&&Number.isFinite(minutes)&&minutes>=0){
      const threshold=[5,10,15,30,60].find(t=>minutes<=t);
      if(threshold){emit('minutes:'+threshold,where+' · About '+Math.max(1,Math.ceil(minutes))+' minutes');for(const t of [5,10,15,30,60])if(t>=threshold)s.seen['minutes:'+t]=true;}
     }
    }
   }
