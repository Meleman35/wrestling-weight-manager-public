...
window.WMOrgInvites=(()=>{
 ...
 function shell(s,body){if(current(s))el('opsContent').innerHTML=`<section class="gs-root oi-root"><h3>Leadership invitations</h3><p>Invite an adult directly to this organization. They use their own account and do not need to join a team.</p><p id="oiStatus" role="status" aria-live="polite"></p>${body}</section>`;}
 async function open({org}){
  close();const s=state={org,epoch,uid:session?.user?.id,team:activeTeam?.id,data:null,pending:null};
 ...
 function render(s){
  if(!current(s))return;
  shell(s,`<div class="ops-actions"><button id="oiNew">Invite organization leader</button><button id="oiRefresh" class="secondary">Refresh</button></div><h4>Organization administrators</h4>${s.data.administrators.map(a=>`<article class="ops-card"><b>${E(a.name)}</b>${a.user_id===s.uid?' · You':`<button class="secondary" data-oi-remove="${E(a.user_id)}">Remove administrator access</button>`}</article>`).join('')}<p class="ops-sheet-note">Position assignments are managed in Positions. Other team roles and access remain separate when a leadership role is removed.</p><h4>Invitations</h4>${s.data.invitations.map(i=>`<article class="ops-card"><b>${E(i.invited_email)}</b><p>${E(i.access_summary.title)} · ${E(i.status)}${i.status==='pending'?' · Expires '+E(new Date(i.expires_at).toLocaleDateString()):''}</p>${i.status==='pending'?`<button class="secondary" data-oi-revoke="${E(i.id)}">Revoke invitation</button>`:''}</article>`).join('')||'<p>No invitations yet.</p>'}`);
