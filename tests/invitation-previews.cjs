...
assert.match(d.querySelector('meta[property="og:image"]').content,/^https:\/\/meleman35\.github\.io\/wrestling-weight-manager-public\/assets\/wrestling-manager-icon\.png$/);
...
const base='https://meleman35.github.io/wrestling-weight-manager-public/';
for(const suffix of ['?invite=WMO-'+ 'a'.repeat(64),'?invite=WMM-private-token','?invite=WMWA-private-token','?invite=WMWG-private-token','?join=TEAM-42','#family-invite=private-family-token','?invite=WMO-private&redirect=https%3A%2F%2Fexample.test%2Fevil']){
 const url=new URL('join.html'+suffix,base);let redirected;
 ...
 const dest=new URL(redirected);assert.equal(dest.origin,url.origin);assert.equal(dest.pathname,'/wrestling-weight-manager-public/');
...
console.log('PASS Seven invitation/query/hash routes preserve the exact private link and stay on the app origin');
...
for(const href of [base, 'file:///app/index.html', 'capacitor://localhost/index.html']){
