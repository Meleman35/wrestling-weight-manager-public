...
(async()=>{
 ...
 const sent=JSON.parse(calls.at(-1).options.body);assert.deepEqual(sent.to,['leader@example.test']);assert.ok(!sent.subject.includes('Override'));assert.ok(!sent.html.includes('<img'));assert.ok(!sent.text.includes('attacker.example.test'));assert.match(sent.text,/https:\/\/theteammanager\.app\/join\.html\?invite=WMO-/);
